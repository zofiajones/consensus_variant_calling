...
#	test_round=float(0.15*float(found_freq[0])+0.2*float(found_freq[1])+0.65*float(found_freq[2]))
	...
	print(test_default_store[ind],'ind0',found_freq0)
	outlier=0
	for i in range(3):
		if int(test_default_store[ind][i]*10) != int(found_freq0[i]*10) :
			outlier=1
	for i in range(len(entry)):
		results[keys[i]].append(entry[i])
	outlier0.append(outlier)
	entry=str(test_default_store[ind][0])+'\t'+str(test_default_store[ind][1])+'\t'+str(test_default_store[ind][2])+'\t'+info+'\n'
	file1.write(entry)


results['outlier0']=outlier0

df=pd.DataFrame(results)
df.to_csv('scores_2.csv',sep="\t")

df1=pd.DataFrame(exceptions)
df1.to_csv('exceptions_2.csv',sep="\t")
